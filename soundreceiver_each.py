...
if __name__ == '__main__':	
	...
	# Number of sample in our playing window.
	data_per_window = 1


	# how many seconds per each point?
	sec_per_sample = data_per_window*1.0/sample_freq
	...
	def callback(in_data, frame_count, time_info, status):
		...
		play_data = queue.pop() if len(queue)>0 else ''.join(map(lambda x : struct.pack('<h',x), np.array([0 for i in range(wav_rate*sec_per_sample)])))
		print len(play_data)
	...
	while True:
		if data is not None and len(data) >= sample_freq*2 :
			print "creating sound at " + str(time.time()-playstamp)

			# Bring two windows w_{i}, w_{i+1}
			itpval = data[:data_per_window*2]			

			# Get x range for interpolation, each points would have (num_data/60)*44100 sound points
			...
			cubicParams = spline(x[:sample_freq],y1)

			# polyval([cn,...,c0],[xn,...,x0]) returns cn * xn^(n-1) + ... + c0 
			...
			ix=xFinelySpaced < sample_freq
			yCombined=concatenate((y1FinelySpaced[ix],y2FinelySpaced[ix==0]))[:(int(wav_rate*sec_per_sample))]
